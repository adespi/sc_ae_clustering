...
class SCDeepCluster(object):
    ...
    def fit(self, x_counts, sf, y, raw_counts, batch_size=256, maxiter=2e4, tol=1e-3, update_interval=140,
        ...
        self.latent_d=self.encoder.predict([x_counts, sf])
        kmeans = KMeans(n_clusters=self.n_clusters, n_init=20)
        #self.y_pred = kmeans.fit_predict(self.encoder.predict([x_counts, sf]))
        self.y_pred = kmeans.fit_predict(self.latent_d)
...
if __name__ == "__main__":
    ...
    x=pd.read_feather("sc_data/snRNA_AD_brain.feather")
...
"""
python -i scDeepCluster_broad_data.py --data_file sc_data/snRNA_AD_brain.feather --n_clusters 7 --pretrain_epochs 12 --maxiter 1
